	...
	for i := 0; i < amount; i++ {
		...
		if source.IsValid() {
			...
			for _, field := range fromTypeFields {
				...
				fieldFlags, ok := tagBitFlags[name]
				if !ok {
					fieldFlags = 0
				}
	...
	if from.IsValid() {
		if to.Kind() == reflect.Ptr {
			...
			if from.Kind() != reflect.Ptr {
				if ifi, ok := from.Interface().(InterfaceI); ok && ifi.Interface() == nil {
					to.Set(reflect.Zero(to.Type()))
					return true
				}
			}
		...
		if from.Type().ConvertibleTo(to.Type()) {
			...
			if rv.Type().AssignableTo(to.Type()) {
				to.Set(rv)
			} else if rv.Type().String() == "[]uint8" && to.Type().String() == "string" {
				// sql.NullString -> string => []uint8 -> string
				to.Set(reflect.ValueOf(string(v.([]byte))))
			}
